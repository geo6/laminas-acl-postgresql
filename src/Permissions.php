...
class Permissions {
  ...
  function __construct() {
    ...
    while ($r = $q->current()) {
      ...
      $q->next(); if ($q->valid() !== TRUE) break;
    }

    $parents = array();
    $q = $this->dbAdapter->query('SELECT r."name" FROM "user" u JOIN "user_role" ur ON u."id" = ur."id_user" JOIN "role" r ON ur."id_role" = r."id" WHERE u."login" = $1 ORDER BY r."priority" ASC', array( $this->login ));
    while ($r = $q->current()) {
      $parents[] = $r->name;
      $q->next(); if ($q->valid() !== TRUE) break;
    }
    ...
    while ($r = $q->current()) {
      ...
      if ($r->public === 't') { $this->acl->allow($this->login, $r->name, 'connect'); }
  ...
  public function getRole($r) {
    if (is_integer($r)) {
      $q = $this->dbAdapter->query('SELECT * FROM "role" WHERE "id" = $1', array( $r ));
    } else {
      $q = $this->dbAdapter->query('SELECT * FROM "role" WHERE "name" = $1', array( $r ));
    }
    return $q->current();
  }
  public function getResource($r) {
    if (is_integer($r)) {
      $q = $this->dbAdapter->query('SELECT * FROM "resource" WHERE "id" = $1', array( $r ));
    } else {
      $q = $this->dbAdapter->query('SELECT * FROM "resource" WHERE "name" = $1', array( $r ));
    }
    return $q->current();
  }
  ...
  public function isAllowed($resource, $privilege = NULL, $relog = FALSE) {
    ...
    if (!in_array(substr($resource, 0, strpos($resource, '-')), array( 'home', 'tools', 'app' ))) $resource = 'app-'.$resource;
    ...
    try {
      ...
      if ($is_allowed !== TRUE && $relog === TRUE) {
        ...
        $q = $dbAdapter->query('SELECT "url" FROM "resource" WHERE "name" = $1 LIMIT 1', array( $resource ));
        $r = $q->current();
  ...
  public function allowPrivilege($role, $resource, $privileges) {
    if (is_null($role)) $role = $this->login;
